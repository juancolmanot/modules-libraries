...
set_datafiles(datafiles_p, n_plots) = sprintf("array datafiles_p[%d] = %s", n_plots * 1, datafiles_p)

eval(set_size(size_p))
eval(set_cols(cols_p, n_plots))
eval(set_labels(labels_p))
eval(set_legends(legends_p, n_plots))
eval(set_datafiles(datafiles_p, n_plots))
...
p for[i=1:n_plots] datafiles_p[i] u cols_p[i * 2 - 1]:cols_p[i * 2] t legends_p[i] linestyle i